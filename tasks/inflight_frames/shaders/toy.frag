...
float sceneSDF(in vec3 worldPos, in mat3 rotationMatrix) {
    ...
    float dynamicLen   = 0.5 * abs(sin(params.iTime));
    float cylinderDist = dCylinder(localPos, 0.1 + 0.75 * dynamicLen, 0.7 + dynamicLen);
    
    float noise         = 0.025 * sin(20.0 * localPos.z + 20.0 * localPos.y - 20.0 * localPos.x + params.iTime * 20.0);
...
void main() {
    ...
    vec2 resolution = vec2(params.iResolution_x, params.iResolution_y);
    vec2 mouse = vec2(params.iMouse_x, params.iMouse_y);
    vec3 mousePos = vec3(mouse / resolution - 0.5, 0.0);
    mat3 rotationMatrix = rotateX(6.0 * mousePos.y) * rotateY(6.0 * mousePos.x);
    
    vec2 scale = 9.0 * resolution / max(resolution.x, resolution.y);
    vec2 uv = scale * (fragCoord / resolution - vec2(0.5));
