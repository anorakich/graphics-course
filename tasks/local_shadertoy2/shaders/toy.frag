...
void main()
{
  ...
  if (hit)
  {
    ...
    vec4 texColor = texture(texture_image, p.xy) * texNorm.z +
                   texture(texture_image, p.xz) * texNorm.y +
                   texture(texture_image, p.yz) * texNorm.x;
    
    vec4 dop = vec4(nl) + sp + vec4(0.3);
    vec4 albedo = vec4(0.5, 0.5, 1, 1) * texColor;
    color = albedo * dop;
  }
